# Pillar Audit: Emotional Regulation

**Date:** 2026-02-05
**Articles:** 7
**Total Word Count:** 18,541
**Status:** FAIL

---

## Article Results

| # | Article | Validation | Links | Citations | Frontmatter | Final |
|---|---------|------------|-------|-----------|-------------|-------|
| 01 | understanding-emotional-regulation-children | FAIL (5) | FAIL | PASS | PASS | FAIL |
| 02 | guide-emotional-regulation-autistic-children | FAIL (5) | FAIL | PASS | PASS | FAIL |
| 03 | emotional-regulation-adhd-children | FAIL (9) | FAIL | PASS | PASS | FAIL |
| 04 | calm-corner-sounds-children | PASS | FAIL | FAIL (1) | PASS | FAIL |
| 05 | co-regulation-through-sound-parent-child | FAIL (6) | FAIL | FAIL (1) | PASS | FAIL |
| 06 | big-feelings-overwhelmed-child-help | FAIL (4) | FAIL | FAIL (2) | PASS | FAIL |
| 07 | complete-guide-emotional-regulation-children | FAIL (21) | FAIL | FAIL (2) | PASS | FAIL |

---

## Issue Categories

| Category | FAILs | WARNs | Articles |
|----------|-------|-------|----------|
| Incorrect Internal Link Format | 71 | 0 | 01, 02, 03, 04, 05, 06, 07 |
| Broken Citation URLs (404) | 3 | 0 | 04, 05, 06, 07 |
| Bot-Blocked Citation URLs (403) | 0 | 6 | 01, 02, 03, 04, 05, 06, 07 |
| Broken Internal Link (slug mismatch) | 2 | 0 | 01, 03 |
| Clinical Terminology in Body | 0 | 7 | 01, 02, 03, 05, 06, 07 |

---

## Link Audit Summary

### Internal Link Format Violations

**Rule:** Internal links must use `/{slug}` format. No directory structures, no file paths, no number prefixes.

Every article in this pillar uses the incorrect format. Links use `/pillar/articles/nn-slug` instead of `/{slug}`.

**Total violations: 71 across all 7 articles.**

#### Article 01: understanding-emotional-regulation-children (9 violations)

**Frontmatter (4):**
- Line 43: `/sensory-overload/articles/01-understanding-sensory-overload-children` → `/understanding-sensory-overload-children`
- Line 45: `/autistic-meltdowns/articles/01-understanding-autism-meltdowns` → `/understanding-autism-meltdowns`
- Line 47: `/adhd-sleep/articles/01-adhd-child-wont-sleep` → `/adhd-child-wont-sleep`
- Line 49: `/calming-sounds/articles/01-why-generic-calming-sounds-dont-work-nd-children` → `/why-generic-calming-sounds-dont-work-nd-children`

**Body (5):**
- Line 112: `/adhd-sleep/articles/01-adhd-child-wont-sleep` → `/adhd-child-wont-sleep`
- Line 114: `/autistic-meltdowns/articles/01-understanding-autism-meltdowns` → `/understanding-autism-meltdowns`
- Line 116: `/sensory-overload/articles/01-understanding-sensory-overload-children` → `/understanding-sensory-overload-children`
- Line 163: `/calming-sounds/articles/01-why-generic-calming-sounds-dont-work-nd-children` → `/why-generic-calming-sounds-dont-work-nd-children`
- Line 207: `/emotional-regulation/articles/02-guide-emotional-regulation-autistic-children` → `/guide-emotional-regulation-autistic-children`

**Additional:** Lines 208-213 contain 5 more internal links using incorrect format to articles 03-07.

#### Article 02: guide-emotional-regulation-autistic-children (8 violations)

**Frontmatter (4):**
- Line 44: `/emotional-regulation/articles/01-understanding-emotional-regulation-children` → `/understanding-emotional-regulation-children`
- Line 46: `/autistic-meltdowns/articles/01-understanding-autism-meltdowns` → `/understanding-autism-meltdowns`
- Line 48: `/sensory-overload/articles/01-understanding-sensory-overload-children` → `/understanding-sensory-overload-children`
- Line 50: `/autistic-meltdowns/articles/07-complete-guide-autism-meltdowns-uk-parents` → `/complete-guide-autism-meltdowns-uk-parents`

**Body (4):**
- Line 85: `/emotional-regulation/articles/01-understanding-emotional-regulation-children` → `/understanding-emotional-regulation-children`
- Line 162: `/sensory-overload/articles/01-understanding-sensory-overload-children` → `/understanding-sensory-overload-children`
- Line 214: `/autistic-meltdowns/articles/07-complete-guide-autism-meltdowns-uk-parents` → `/complete-guide-autism-meltdowns-uk-parents`
- Line 256: `/emotional-regulation/articles/07-complete-guide-emotional-regulation-children` → `/complete-guide-emotional-regulation-children`

#### Article 03: emotional-regulation-adhd-children (13 violations)

**Frontmatter (4):**
- Line 43: `/emotional-regulation/articles/01-understanding-emotional-regulation-children` → `/understanding-emotional-regulation-children`
- Line 45: `/adhd-sleep/articles/01-adhd-child-wont-sleep` → `/adhd-child-wont-sleep`
- Line 47: `/adhd-sleep/articles/04-adhd-racing-thoughts` → `/adhd-racing-thoughts`
- Line 49: `/adhd-sleep/articles/03-calming-sounds-adhd` → `/calming-sounds-adhd`

**Body (9):**
- Line 78: `/emotional-regulation/articles/01-understanding-emotional-regulation-children` → `/understanding-emotional-regulation-children`
- Line 172: `/adhd-sleep/articles/03-calming-sounds-adhd` → `/calming-sounds-adhd`
- Line 204: `/emotional-regulation/articles/05-co-regulation-through-sound-parent-child` → `/co-regulation-through-sound-parent-child`
- Line 223: `/adhd-sleep/articles/01-adhd-child-wont-sleep` → `/adhd-child-wont-sleep`
- Line 223: `/adhd-sleep/articles/04-adhd-racing-thoughts` → `/adhd-racing-thoughts`
- Line 245: `/emotional-regulation/articles/02-guide-emotional-regulation-autistic-children` → `/guide-emotional-regulation-autistic-children`
- Line 246: `/emotional-regulation/articles/04-calm-corner-sounds-children` → `/calm-corner-sounds-children`
- Line 247-248: 2 more internal links using incorrect format
- Line 250: `/emotional-regulation/articles/07-complete-guide-emotional-regulation-children` → `/complete-guide-emotional-regulation-children`

#### Article 04: calm-corner-sounds-children (6 violations)

**Frontmatter (3):**
- Line 42: `/emotional-regulation/articles/01-understanding-emotional-regulation-children` → `/understanding-emotional-regulation-children`
- Line 44: `/calming-sounds/articles/01-why-generic-calming-sounds-dont-work-nd-children` → `/why-generic-calming-sounds-dont-work-nd-children`
- Line 46: `/calming-sounds/articles/04-calming-sounds-every-situation-anxiety-focus-transitions` → `/calming-sounds-every-situation-anxiety-focus-transitions`

**Body (3):**
- Line 93: `/emotional-regulation/articles/01-understanding-emotional-regulation-children` → `/understanding-emotional-regulation-children`
- Line 193: `/calming-sounds/articles/01-why-generic-calming-sounds-dont-work-nd-children` → `/why-generic-calming-sounds-dont-work-nd-children`
- Line 253: `/calming-sounds/articles/04-calming-sounds-every-situation-anxiety-focus-transitions` → `/calming-sounds-every-situation-anxiety-focus-transitions`

#### Article 05: co-regulation-through-sound-parent-child (7 violations)

**Frontmatter (3):**
- Line 43: `/emotional-regulation/articles/01-understanding-emotional-regulation-children` → `/understanding-emotional-regulation-children`
- Line 45: `/emotional-regulation/articles/04-calm-corner-sounds-children` → `/calm-corner-sounds-children`
- Line 47: `/calming-sounds/articles/01-why-generic-calming-sounds-dont-work-nd-children` → `/why-generic-calming-sounds-dont-work-nd-children`

**Body (4):**
- Line 122: `/calming-sounds/articles/01-why-generic-calming-sounds-dont-work-nd-children` → `/why-generic-calming-sounds-dont-work-nd-children`
- Line 206: `/emotional-regulation/articles/04-calm-corner-sounds-children` → `/calm-corner-sounds-children`
- Line 222: `/emotional-regulation/articles/01-understanding-emotional-regulation-children` → `/understanding-emotional-regulation-children`
- Line 230: `/emotional-regulation/articles/07-complete-guide-emotional-regulation-children` → `/complete-guide-emotional-regulation-children`

#### Article 06: big-feelings-overwhelmed-child-help (7 violations)

**Frontmatter (3):**
- Line 42: `/emotional-regulation/articles/01-understanding-emotional-regulation-children` → `/understanding-emotional-regulation-children`
- Line 44: `/emotional-regulation/articles/04-calm-corner-sounds-children` → `/calm-corner-sounds-children`
- Line 46: `/autistic-meltdowns/articles/01-understanding-autism-meltdowns` → `/understanding-autism-meltdowns`

**Body (4):**
- Line 97: `/emotional-regulation/articles/01-understanding-emotional-regulation-children` → `/understanding-emotional-regulation-children`
- Line 190: `/emotional-regulation/articles/04-calm-corner-sounds-children` → `/calm-corner-sounds-children`
- Line 192: `/autistic-meltdowns/articles/01-understanding-autism-meltdowns` → `/understanding-autism-meltdowns`
- Line 276: `/emotional-regulation/articles/07-complete-guide-emotional-regulation-children` → `/complete-guide-emotional-regulation-children`

#### Article 07: complete-guide-emotional-regulation-children (21 violations)

**Frontmatter (10):**
- Lines 46-64: All 10 internal links in frontmatter use directory structure format

**Body (11):**
- Line 182: `/emotional-regulation/articles/01-understanding-emotional-regulation-children` → `/understanding-emotional-regulation-children`
- Line 240: `/emotional-regulation/articles/06-big-feelings-overwhelmed-child-help` → `/big-feelings-overwhelmed-child-help`
- Line 262: `/emotional-regulation/articles/02-emotional-regulation-autistic-children` → `/guide-emotional-regulation-autistic-children`
- Line 264: `/autistic-meltdowns/articles/01-understanding-autism-meltdowns` → `/understanding-autism-meltdowns`
- Line 278: `/emotional-regulation/articles/03-emotional-regulation-adhd-children` → `/emotional-regulation-adhd-children`
- Line 280: `/adhd-sleep/articles/01-adhd-child-wont-sleep` → `/adhd-child-wont-sleep`
- Line 330: `/calming-sounds/articles/07-complete-guide-calming-sounds-children` → `/complete-guide-calming-sounds-children`
- Line 344: `/emotional-regulation/articles/04-calm-corner-sounds-children` → `/calm-corner-sounds-children`
- Line 364: `/sensory-overload/articles/01-understanding-sensory-overload-children` → `/understanding-sensory-overload-children`
- Line 418: `/emotional-regulation/articles/05-co-regulation-through-sound-parent-child` → `/co-regulation-through-sound-parent-child`
- Line 465: `/adhd-sleep/articles/01-adhd-child-wont-sleep` → `/adhd-child-wont-sleep`

### Broken Internal Links (Slug Mismatch)

| Article | Link Used | Actual Slug | Issue |
|---------|-----------|-------------|-------|
| 01 (line 207) | `/emotional-regulation/articles/02-guide-emotional-regulation-autistic-children` | `guide-emotional-regulation-autistic-children` | File is `02-emotional-regulation-autistic-children.md` but link says `02-guide-emotional-regulation-autistic-children` |
| 03 (line 245) | `/emotional-regulation/articles/02-guide-emotional-regulation-autistic-children` | `guide-emotional-regulation-autistic-children` | Same mismatch as article 01 |

**Note:** Article 02's frontmatter slug is `guide-emotional-regulation-autistic-children` but the filename is `02-emotional-regulation-autistic-children.md`. The internal links from articles 01 and 03 reference a path that doesn't match either the slug or the filename.

### Cross-Pillar Links

| Direction | Count | Target Pillars |
|-----------|-------|----------------|
| Outbound | 12 | adhd-sleep (5), autistic-meltdowns (4), sensory-overload (3), calming-sounds (5) |
| Inbound | Not checked (would require reading other pillars) |

### Pillar Guide Coverage

| Check | Value | Status |
|-------|-------|--------|
| Guide → Supporting (body links) | 6/6 | PASS |
| Supporting → Guide (body links) | 6/6 | PASS |

All supporting articles link to the pillar guide. The pillar guide links to all supporting articles.

---

## External Citations

| Status | Count | Details |
|--------|-------|---------|
| Working (200) | 5 | autism.org.uk, gov.uk (x2), additudemag.com, nhs.uk/adhd |
| Bot-Blocked (403) | 6 | journals.sagepub.com, ncbi.nlm.nih.gov (x4), mind.org.uk |
| Broken (404) | 3 | zonesofregulation.com, nhs.uk/emotions, hushaway.com/the-open-sanctuary |
| Redirects | 0 | - |
| Timeout | 0 | - |

### Broken URLs (404) - Manual Review Required

| URL | Articles | Notes |
|-----|----------|-------|
| `https://www.zonesofregulation.com/learn-more.html` | 04, 07 | Page removed or restructured |
| `https://www.nhs.uk/mental-health/children-and-young-adults/advice-for-parents/help-your-child-manage-emotions/` | 06, 07 | NHS page moved or removed |
| `https://hushaway.com/the-open-sanctuary/` | 05 | HushAway page not yet live or URL changed |

### Bot-Blocked URLs (403) - Likely Valid

These URLs return 403 to automated requests (bot protection) but are likely valid academic/charity sources:

| URL | Articles |
|-----|----------|
| `https://journals.sagepub.com/doi/10.1177/1362361319877792` | 02, 07 |
| `https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4282137/` | 03, 07 |
| `https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5618809/` | 04, 05 |
| `https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4774859/` | 01, 06, 07 |
| `https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3680170/` | 05, 07 |
| `https://www.mind.org.uk/information-support/for-children-and-young-people/emotions/` | 01, 06 |

**Domain Frequency:** No article exceeds the 2-citation-per-domain limit within a single article.

---

## Frontmatter Accuracy

| Article | word_count | keyword_density | Keywords | Slug |
|---------|------------|-----------------|----------|------|
| 01-understanding-emotional-regulation-children | PASS (2418) | PASS (1.3%) | PASS | PASS |
| 02-emotional-regulation-autistic-children | PASS (2312) | PASS (1.4%) | PASS | PASS |
| 03-emotional-regulation-adhd-children | PASS (2398) | PASS (1.4%) | PASS | PASS |
| 04-calm-corner-sounds-children | PASS (2456) | PASS (1.4%) | PASS | PASS |
| 05-co-regulation-through-sound-parent-child | PASS (2047) | PASS (1.4%) | PASS | PASS |
| 06-big-feelings-overwhelmed-child-help | PASS (2387) | PASS (1.4%) | PASS | PASS |
| 07-complete-guide-emotional-regulation-children | PASS (4523) | PASS (1.4%) | PASS | PASS |

---

## Cross-Article Consistency

### Terminology Variations

| Term Cluster | Variations | Articles | Action |
|--------------|------------|----------|--------|
| dysregulation/overwhelm | "emotional dysregulation" + "overwhelm" used interchangeably | All | WARN - Profile says avoid "dysregulation" (clinical), use "overwhelm" or "big feelings". Articles use both. Consider standardising to "overwhelm" in parent-facing text, keeping "dysregulation" only in FAQ/SEO keyword contexts. |
| intervention/support | "intervention" used in article 07 line 510 | 07 | WARN - Profile lists "intervention" as a word to avoid. Replace with "support" or "help". |

### Conflicting Claims

No conflicting factual claims found across articles. Statistics and research references are consistent.

### Positioning Alignment

| Article | Assigned Angle | Alignment |
|---------|---------------|-----------|
| 01 | It's neurology, not behaviour | Strong |
| 02 | Generic strategies fail autistic children | Strong |
| 03 | Tools must work WITH the ADHD brain | Strong |
| 04 | Calm corners are missing sound (conversion driver) | Strong |
| 05 | Sound regulates both parent and child | Strong |
| 06 | Sound first in the first 60 seconds | Strong |
| 07 | Sound is the missing tool in regulation | Strong |

All articles align well with their assigned positioning angles from `02-positioning.md`.

---

## Patterns (3+ occurrences)

### Incorrect Internal Link Format
- Articles: 01, 02, 03, 04, 05, 06, 07
- Count: 71
- Recommend for common-mistakes.md: Yes (already documented as rule 5a)

### Clinical Term "dysregulation" in Parent-Facing Content
- Articles: 01, 02, 03, 05, 06, 07
- Count: 30+ uses across pillar
- Recommend for common-mistakes.md: No (borderline - used alongside accessible language, and appears in SEO keywords)

---

## Summary of Issues Requiring Manual Review

### Must Fix (FAIL)

1. **Internal link format (71 instances across all 7 articles):** All internal links must be converted from `/pillar/articles/nn-slug` to `/{slug}` format. This is an auto-fixable issue.

2. **Broken internal link slug mismatch (2 instances in articles 01, 03):** Links reference `02-guide-emotional-regulation-autistic-children` but the file is `02-emotional-regulation-autistic-children.md`. Need to verify correct slug and fix.

### Cannot Auto-Fix (Escalate)

3. **Broken citation URL: zonesofregulation.com (articles 04, 07):** Page returns 404. Find replacement URL or remove citation.

4. **Broken citation URL: nhs.uk/emotions (articles 06, 07):** NHS page returns 404. Find replacement URL.

5. **Broken citation URL: hushaway.com/the-open-sanctuary (article 05):** Returns 404. Verify correct URL when site is live.

### Should Fix (WARN)

6. **Clinical term "intervention" (article 07, line 510):** Replace with "support" per brand voice rules.

7. **Clinical term "dysregulation" used extensively:** Consider reducing in parent-facing body text, keeping only in FAQ/SEO contexts.

---

*Audit completed 2026-02-05. Validate-only mode (no fixes applied).*
